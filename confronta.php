    ...
    <script>
        ...
        // Dividiamo la stringa dei cookie in un array e iteriamo su ciascun cookie
            document.cookie.split(";").forEach(function(c) {
                ...
                if (cookie[0].indexOf('PHPSESSID') === -1) { // Escludiamo i cookie di sessione
...
<body>
<?php
    // Includiamo l'header del sito web
    include 'header.php';
    // Controlliamo se l'utente è loggato, se non è loggato reindirizziamo alla pagina di login
    if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] === false) {
        header('Location: login.php');
        exit;
    }
    // Inizializzamo un array vuoto per raccogliere le auto selezionate
    $auto_selezionate = [];

    // Scorriamo tutti i cookie per trovare quelli che iniziano con 'autoConfronto_' e li aggiungiamo all'array
    ...
    // Stabiliamo una connessione al database PostgreSQL
    require "tswdb.php";
    $db = pg_connect($connection_string) or die('Impossibile connettersi al database: ' . pg_last_error());

    // Se ci sono auto selezionate, esegui una query per recuperare i loro dettagli
    if (count($auto_selezionate) > 0) {
        // Assicuriamoci che gli ID siano numeri interi per prevenire SQL injection
